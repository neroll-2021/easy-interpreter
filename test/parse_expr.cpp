...
int main() {
    ...
    auto result = node->evaluate();
    if (result->type() == nsd::variable_type::error) {
        std::println("error type");
        auto p = std::dynamic_pointer_cast<nsd::error_value>(result);
        std::println("{}", p->value());
        return 0;
    }
    if (result->type() == nsd::variable_type::integer) {
        auto p = std::dynamic_pointer_cast<nsd::int_value>(result);
        std::println("{}", p->value());
    } else if (result->type() == nsd::variable_type::floating) {
        auto p = std::dynamic_pointer_cast<nsd::float_value>(result);
        std::println("{}", p->value());
    } else if (result->type() == nsd::variable_type::boolean) {
        auto p = std::dynamic_pointer_cast<nsd::boolean_value>(result);
        std::println("{}", p->value());
    }
}